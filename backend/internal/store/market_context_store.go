...
func (s *Store) SaveMarketContext(ctx context.Context, market ecb.MarketContext) error {
	...
	metricStmt, err := tx.PrepareContext(ctx, `
		INSERT INTO market_metrics (code, label, category, value, unit, observed_on, source_url, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	...
	for _, m := range market.Metrics {
		if _, err := metricStmt.ExecContext(ctx, m.Code, m.Label, m.Category, m.Value, m.Unit, m.ObservedOn, m.SourceURL, now); err != nil {
			return err
		}
	}
...
func (s *Store) ListMarketMetrics(ctx context.Context) ([]ecb.Metric, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT code, label, category, value, unit, observed_on, source_url
		FROM market_metrics
	...
	for rows.Next() {
		var m ecb.Metric
		if err := rows.Scan(&m.Code, &m.Label, &m.Category, &m.Value, &m.Unit, &m.ObservedOn, &m.SourceURL); err != nil {
			return nil, err
		}
